...
def parse_df_times(df):
    parsed_df = df.copy()
    # Split the 'start_time' and 'end_time' columns into 'start_day', 'start_time', 'end_day', and 'end_time'
    parsed_df["start_day"] = parsed_df["start_time"].str.split(".", expand=True)[0]
    parsed_df["end_day"] = parsed_df["end_time"].str.split(".", expand=True)[0]
    parsed_df["start_time"] = parsed_df["start_time"].str.split(".", expand=True)[1]
    parsed_df["end_time"] = parsed_df["end_time"].str.split(".", expand=True)[1]
    ...
    # Add the number of days to the 'end_time' column if the difference is positive
...
def get_duty_times(df):
    ...
    # Convert the 'start_time' and 'end_time' columns to time format
